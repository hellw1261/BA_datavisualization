...
const routers: any[] = formatRoute(
  permissionStore.getRoutersNotHidden.filter(
    route => !route.path.includes('/screen')
...
<template>
  <el-header class="header-flex" :class="{ 'header-light': navigateBg === 'light' }">
    ...
    >
      ...
      <HeaderMenuItem
        v-for="menu in routers.filter(menu => !menu.path.includes('template'))"
        :key="menu.path"
        :menu="menu"
      />
    </el-menu>
